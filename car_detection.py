...
class MovingVehicleDetector:
    ...
    def __init__(
        ...
        # Consider it a new car if more than 3 seconds have passed
        self.car_event_timeout = 3.0
    ...
    def _save_detection(self, frame: np.ndarray, result, motion_mask: np.ndarray) -> None:
        ...
        try:
            ...
            if (self.current_car_folder is None or
                ...
                self.current_car_folder = self._create_new_car_folder(
                    timestamp)
                self.logger.info(
    ...
    def _upload_to_roboflow(self, frame: np.ndarray, timestamp: str) -> None:
        ...
        Args:
            frame: The image frame to upload
            timestamp: Timestamp string for the image
        """
        if not self.roboflow_project:
            return

        try:
            # Use the file that was just saved in _save_detection
